...
function populateAMCOLTrainingHydroOptions(){const sel=document.getElementById('hydroDataPackSelect');if(!sel)return;if(sel.querySelector('optgroup[data-amcol-training]'))return;const real=document.createElement('optgroup');real.label='REAL VESSELS · SOURCE-ANCHORED AMCOL CALIBRATED HYDRO + KN';real.dataset.realCalibrated='1';const g=document.createElement('optgroup');g.label='AMCOL TRAINING VESSELS · hydrostatics + KN';g.dataset.amcolTraining='1';(AMCOL_TRAINING_VESSELS_MASTER.vessels||[]).forEach(v=>{const o=document.createElement('option');o.value=amcolTrainingHydroKey(v.id);o.textContent=v.realSourceCalibrated?`${v.companyName} · ${v.name} · CALIBRATED hydro + KN`:`${v.name} · AMCOL TRAINING hydro + KN`;if(v.realSourceCalibrated)real.appendChild(o);else g.appendChild(o);});if(real.children.length)sel.appendChild(real);sel.appendChild(g);}
function loadAMCOLTrainingVessel(id){
 const v=AMCOL_TRAINING_VESSELS_BY_ID[id];if(!v)return false;const pp=v.principalParticulars||{},condition=(v.loadingConditions||[])[0]||{};
 prepareReferenceVesselLoad();cancelStabilityTestRuntime();vesselVisualTransaction=true;
 try{
  resetCore();
  state.amcolTrainingVesselId=v.id;state.companyName=v.realSourceCalibrated?(v.companyName||'Source company'):'Asian Maritime Technological College (AMCOL)';state.vesselName=v.name;state.hullType=v.family;
  state.length=+pp.LBP||100;state.beam=+pp.beam||20;state.depth=+pp.depth||10;state.density=+v.sourceDensity||+pp.waterDensity||1.025;state.visualReferenceDraft=Number.isFinite(+pp.designDraft)?+pp.designDraft:null;state.visualReferenceDraftSource=v.realSourceCalibrated?'Published summer draught anchor · AMCOL calibrated model':'AMCOL training design draft';
  ...
  window.AMCOL_CUSTOM_HULL_FORM={enabled:true,trainingModel:true,label:v.realSourceCalibrated?`${v.name} · source-anchored AMCOL calibrated station envelope`:`${v.name} · AMCOL training station envelope`,vesselName:v.name,hullType:v.family,stations:(v.stationEnvelope||[]).map(s=>({xNorm:(+s.xNorm||0)*2,beamFactor:+s.beamFactor,bottomFactor:+s.bottomFactor,sheerRatio:+s.sheerRatio||0,keelRiseRatio:+s.keelRiseRatio||0}))};
  ...
  ballastTanks=amcolTrainingBallastPlan(v,condition);state.ballastPlanEnabled=true;state.ballastPlanSource='training';state.ballastPlanLabel=v.realSourceCalibrated?`${v.companyName} · ${v.name} · AMCOL calibrated representative ballast plan · ${ballastTanks.length} tanks`:`${v.name} · AMCOL TRAINING MODEL ballast plan · ${ballastTanks.length} tanks`;
  ...
  const ol=v.operationalLimits||{};operationalLimits={enabled:true,source:v.realSourceCalibrated?'real-vessel-amcol-calibrated':'amcol-training',label:v.realSourceCalibrated?'AMCOL CALIBRATED TRAINING LIMITS · not statutory':'AMCOL TRAINING LIMITS · not statutory',minForwardDraft:Number.isFinite(+ol.minForwardDraft)?+ol.minForwardDraft:null,minAftDraft:Number.isFinite(+ol.minAftDraft)?+ol.minAftDraft:null,maxDraft:Number.isFinite(+ol.maxDraft)?+ol.maxDraft:null,minUKC:Number.isFinite(+ol.minUKC)?+ol.minUKC:null,maxList:Number.isFinite(+ol.maxList)?+ol.maxList:null,maxTrim:Number.isFinite(+ol.maxTrim)?+ol.maxTrim:null,airDraft:Number.isFinite(+ol.airDraft)?+ol.airDraft:null};
 ...
 commitVesselVisualRefresh('amcol-training-vessel-load');showCleanFeedback(v.realSourceCalibrated?`${v.companyName} · ${v.name}: source-anchored AMCOL calibrated hydrostatics, KN, cargo and ballast training data loaded.`:`${v.name}: complete AMCOL training hydrostatics, KN, cargo and ballast data loaded.`);return true;
}
...
function populateReferenceVesselSelect(){
 ...
 Object.entries(referenceVesselPresets).filter(([,r])=>r.amcolTrainingId&&!AMCOL_TRAINING_VESSELS_BY_ID[r.amcolTrainingId]?.realSourceCalibrated).forEach(([key,r])=>{const v=AMCOL_TRAINING_VESSELS_BY_ID[r.amcolTrainingId],pp=v?.principalParticulars||{};const o=document.createElement('option');o.value=key;o.textContent=`${r.vesselName} · ${v?.typeLabel||r.hullType} — L ${(+r.length).toFixed(1)} × B ${(+r.beam).toFixed(1)} × D ${(+r.depth).toFixed(1)} m · HYDRO + KN + TANKS`;train.appendChild(o);});
...
function mainHullStationEnvelopeAt(xNorm,type=state.hullType){
 xNorm=Math.max(-1,Math.min(1,Number(xNorm)||0));const custom=window.AMCOL_CUSTOM_HULL_FORM;if(custom?.enabled&&Array.isArray(custom.stations)&&custom.stations.length>=5&&(!custom.vesselName||custom.vesselName===state.vesselName)){const pts=custom.stations.map(s=>[+s.xNorm,+s.beamFactor,+s.bottomFactor,+s.sheerRatio||0,+s.keelRiseRatio||0]).filter(a=>a.every(Number.isFinite)).sort((a,b)=>a[0]-b[0]),r=interpolateMainHullEnvelope(pts,xNorm);if(r)return {beamFactor:r[1],bottomFactor:r[2],sheerRatio:r[3],keelRiseRatio:r[4],source:'custom'};}
...
function calculateAll({curve=true}={}){
 ...
 updatePhysicsValidity();renderBallastPlan();renderDataAuthority();renderPhysicsIntegrity();if(document.getElementById('cargoManagerBackdrop')&&!document.getElementById('cargoManagerBackdrop').classList.contains('hidden'))renderCargoManager();
 state.operationalGZ=operationalGZAt(state.heel);state.operationalRM=state.dispMass*G*state.operationalGZ;
 updateUI();
}
...
function requestLiveCalculation(delay=70){clearTimeout(liveCalcTimer);liveCalcTimer=setTimeout(()=>{liveCalcTimer=null;calculateAll();},delay);}
function bind(id,key,parser=parseFloat){
 ...
 el.addEventListener('change',()=>{state[key]=parser(el.value);clearTimeout(liveCalcTimer);liveCalcTimer=null;if(!dimensionField)calculateAll();});
}
...
const DRAFT_SURVEY_TRAINING_MISSIONS={
  ...
  dock_water:{title:'Mission D · Dock-Water Top-Off',level:'Advanced',vesselId:'AMCOL-FORTUNE',initialCondition:2,finalCondition:0,initialDensity:1.0100,finalDensity:1.0180,sideDelta:.008,operation:'A top-off loading operation is surveyed in different dock-water densities. Apply density correction, tank soundings and changing deductibles before reporting cargo loaded.'}
};
...
function draftSurveyMissionSubmit(){
  ...
  draftSurveyMissionState.grade=AMCOLPhysics.draftSurveyMission.gradeMission({truth,entered});renderDraftSurveyMissionPanel();
}
...
function initCleanUI(){
 ...
 if(data&&!document.getElementById('dataCompletenessCard')){
  ...
  const lib=document.createElement('details');lib.id='conditionLibraryCard';lib.className='rounded-xl border border-slate-800 bg-slate-950/45 p-3';lib.innerHTML='<summary class="cursor-pointer font-bold text-cyan-300">Condition Library · Save / Compare</summary><div class="mt-3 grid md:grid-cols-3 gap-2"><input id="conditionNameInput" placeholder="Condition name" class="md:col-span-2 bg-slate-950 border border-slate-700 rounded px-2 py-1.5 text-slate-200"><button onclick="saveCurrentCondition()" class="rounded bg-cyan-500/15 border border-cyan-500/30 text-cyan-300 font-bold">Save Current</button><select id="conditionLibrarySelect" class="md:col-span-2 bg-slate-950 border border-slate-700 rounded px-2 py-1.5 text-slate-200"><option value="">Choose saved condition…</option></select><div id="conditionLibraryCount" class="text-right text-[8px] text-slate-500 self-center">0/12 saved</div><button onclick="loadSelectedCondition()" class="rounded border border-emerald-700/40 text-emerald-300 py-1.5">Load</button><button onclick="compareSelectedCondition()" class="rounded border border-amber-700/40 text-amber-300 py-1.5">Compare</button><button onclick="deleteSelectedCondition()" class="rounded border border-rose-800/40 text-rose-300 py-1.5">Delete</button></div><div id="conditionCompareReadout" class="mt-3 text-[9px] text-slate-300"></div>';data.appendChild(lib);
  const diag=document.createElement('details');diag.id='physicsValidationCard';diag.className='rounded-xl border border-slate-800 bg-slate-950/45 p-3 advanced-only';diag.innerHTML='<summary class="cursor-pointer font-bold text-violet-300">Physics & Challenge Regression Diagnostics</summary><div class="mt-3 grid grid-cols-3 gap-2"><button onclick="runPhysicsValidationSuite()" class="py-1.5 rounded bg-violet-500/10 border border-violet-500/30 text-violet-300 font-bold">Core Physics</button><button onclick="runVesselDataRegressionSuite()" class="py-1.5 rounded bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 font-bold">Vessel Data</button><button onclick="runChallengeRegressionSuite()" class="py-1.5 rounded bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 font-bold">25 Challenges</button></div><div id="physicsValidationReadout" class="mt-2 space-y-1 text-[9px] text-slate-300">Checks GREAT FORTUNE targets and transverse-force sign symmetry, then restores the current condition.</div><div id="vesselDataRegressionReadout" class="mt-2 space-y-1 text-[9px] text-slate-300">Checks every embedded vessel for hydro monotonicity, KN↔KMT consistency, loading mass closure and FWD/AFT trim closure.</div><div id="challengeRegressionReadout" class="mt-2 space-y-1 text-[9px] text-slate-300">Challenge regression loads each challenge, applies its reference solution and verifies the current PASS logic.</div>';data.appendChild(diag);
 }
